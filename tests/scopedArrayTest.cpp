...
TEST_F(ScopedArrayTest, ConstructorDestructorGet)
{
    ...
    EXPECT_NO_THROW({
        AllocationTracker* direct = new AllocationTracker[100];
...
TEST_F(ScopedArrayTest, Operators)
{
    ...
    const ScopedArray<AllocationTracker> array2(directArray);

    EXPECT_TRUE(array2);
    for (uint32_t i = 0; i < 99; ++i) {
        EXPECT_EQ(&array2[i], &directArray[i]);
...
TEST_F(ScopedArrayTest, Compilation)
{
    ...
    ScopedArray<int> b(0); // This line is correct
    // ScopedArray<int> c = new int[100];  // This line is incorrect
    // ScopedArray<int> d = 0;             // This line is incorrect
